 ...
 * adicionando novas funcionalidades e alguns extras, tal como
...
// inicializando variáveis de login e session
...
// armazenando elementos em variáveis para fácil acesso
...
// adiciona eventlistener no botão de login
...
// adiciona eventlistener no botão de pesquisa
  ...
  for (const item of listaDeFilmes.results) {
    ...
    const content = `
        <div class="card--effect card mb-3 position-relative" style="width: 540px; height:180px">
                <i onclick="adicionarFilmeNaLista(${
                  item.id
                }, ${listId})" class="add-to-list bi bi-bookmark-plus fs-5 position-absolute"></i>
            ...
            <div class="col-md-8">
                <div class="card-body">
                    <h5 class="card-title">${
                      item.original_title
                    }, (${item.release_date.substring(0, 4)})</h5>
...
const criarLista = async () => {
  var nomeDaLista = (document.getElementById("nome-lista") as HTMLInputElement)!
    .value;
...
const adicionarFilmeNaLista = async (filmeId: number, listaId: string) => {
  ...
  (document.getElementById(
    "list-selector"
  ) as HTMLSelectElement)!.selectedIndex = 1;
};
...
const pegarLista = async (listaId?: string) => {
  ...
  for (let i = 0; i < result.items.length; i++) {
    ...
    let listPosterUrl =
      "https://image.tmdb.org/t/p/w92" + result.items[i].poster_path;
    const listContent = `
